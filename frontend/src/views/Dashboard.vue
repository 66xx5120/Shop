...
<div class="card">
  <h3>服务/备注消费查询</h3>
  <div class="form-row">
    <input type="date" v-model="serviceRemarkQuery.startDate" />
    <input type="date" v-model="serviceRemarkQuery.endDate" />
    <select v-model="serviceRemarkQuery.serviceTypeId">
      <option value="">全部服务</option>
      <option v-for="s in services" :key="s.id" :value="s.id">{{ s.name }}</option>
    </select>
    <input v-model="serviceRemarkQuery.remarkKeyword" placeholder="备注关键字（模糊）" />
    <button class="primary-btn" @click="searchServiceRemarkConsume">查询</button>
    <button class="secondary-btn" @click="exportServiceRemarkConsume">导出CSV</button>
  </div>
  <div v-if="serviceRemarkConsume.items.length">
    <p>总消费金额：{{ money(serviceRemarkConsume.summary.totalAmount) }} | 总订单数：{{ serviceRemarkConsume.summary.totalCount }}</p>
    <div class="table-wrap">
      <table>
        <thead>
          <tr><th>时间</th><th>会员姓名</th><th>服务名称</th><th>金额</th><th>备注</th> </tr>
        </thead>
        <tbody>
          <tr v-for="item in serviceRemarkConsume.items" :key="item.id">
            <td>{{ formatDate(item.createdAt) }}</td>
  ...
  <div v-else-if="serviceRemarkSearched && !serviceRemarkConsume.items.length" class="muted">未找到符合条件的消费记录</div>
</div>
      </div>
...
async function saveCustomer() {
  ...
  // 手机号格式校验：11位数字
  const phoneRegex = /^1[3-9]\d{9}$/;  // 简单校验：1开头，第二位3-9，后面9位数字，共11位
  if (!phoneRegex.test(customerForm.phone)) {
    return showToast('手机号格式不正确，请正确输入11位数字手机号');
  }
  ...
  try {
    ...
    if (customerForm.id) {
      await axios.put(`/api/customers/${customerForm.id}`, payload);
      showToast('更新成功');
    } else {
...
// 服务/备注消费查询
const serviceRemarkQuery = reactive({
  startDate: new Date(Date.now() - 30 * 86400000).toISOString().slice(0, 10),
  endDate: new Date().toISOString().slice(0, 10),
  serviceTypeId: '',
  remarkKeyword: ''
});
const serviceRemarkConsume = reactive({
  items: [],
  summary: { totalAmount: 0, totalCount: 0 }
});
const serviceRemarkSearched = ref(false);

async function searchServiceRemarkConsume() {
  serviceRemarkSearched.value = true;
  try {
    const res = await axios.get('/api/reports/service-remark-consume', {
      params: {
        startDate: serviceRemarkQuery.startDate,
        endDate: serviceRemarkQuery.endDate,
        serviceTypeId: serviceRemarkQuery.serviceTypeId,
        remarkKeyword: serviceRemarkQuery.remarkKeyword
      }
    });
    if (res.data.success) {
      serviceRemarkConsume.items = res.data.data.items;
      serviceRemarkConsume.summary = res.data.data.summary;
    } else {
      showToast(res.data.message || '查询失败');
    }
  } catch (err) {
...
async function exportServiceRemarkConsume() {
  try {
    const res = await axios.get('/api/export/service-remark-consume', {
      params: {
        startDate: serviceRemarkQuery.startDate,
        endDate: serviceRemarkQuery.endDate,
        serviceTypeId: serviceRemarkQuery.serviceTypeId,
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `service_remark_consume_${serviceRemarkQuery.startDate}_to_${serviceRemarkQuery.endDate}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  } catch (err) {
    showToast('导出失败');
